...
public class ExtractionDependentOccurrenceExtractor extends RecommendationExtractor {
	...
	private void searchForName(IWord n) {
		List<IInstance> instances = modelState.getInstances().stream().filter(i -> SimilarityUtils.areWordsOfListsSimilar(i.getNames(), List.of(n.getText())))
	...
	private void searchForType(IWord n) {
		List<IInstance> instances = modelState.getInstances().stream().filter(i -> SimilarityUtils.areWordsOfListsSimilar(i.getTypes(), List.of(n.getText())))
				.collect(Collectors.toList());
		if (instances.size() == 1) {
