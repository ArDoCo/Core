...
public class BabelNetHttpApi {
    ...
    private static final String WORD_SENSE_QUERY = "https://babelnet.io/v6/getSynsetIds" + "?lemma={lemma}" + "&searchLang={searchLang}"
            + "&targetLang={targetLang}" + "&key={key}";
    ...
    public String querySynsetIdsOfLemma(String lemma) throws IOException, InterruptedException, BabelNetRequestLimitException, BabelNetInvalidKeyException {
        ...
        String url = WORD_SENSE_QUERY.replace("{lemma}", lemma)
    ...
    private String sendGetRequest(HttpRequest request) throws IOException, InterruptedException, BabelNetInvalidKeyException, BabelNetRequestLimitException {
