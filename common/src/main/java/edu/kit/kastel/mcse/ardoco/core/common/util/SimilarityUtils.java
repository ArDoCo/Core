...
public final class SimilarityUtils {
	...
	/**
	 * Compares a given {@link INounMapping} with a given {@link IModelInstance} for
	 * similarity. Checks if all names, the longest name or a single name are
	 * similar to the reference of the NounMapping.
	 *
	 * @param nounMapping the {@link INounMapping}
	 * @param instance    the {@link IModelInstance}
	 * @return true, iff the {@link INounMapping} and {@link IModelInstance} are
	 *         similar.
	 */
	public static boolean isNounMappingSimilarToModelInstance(INounMapping nounMapping, IModelInstance instance) {
		if (areWordsOfListsSimilar(instance.getNameParts(), Lists.immutable.with(nounMapping.getReference())) || areWordsSimilar(instance.getFullName(), nounMapping.getReference())) {
			return true;
		}
	...
	/**
	 * Compares a given {@link IWord} with a given {@link IModelInstance} for
	 * similarity.
	 *
	...
	/**
	 * Compares a given {@link IRecommendedInstance} with a given
	 * {@link IModelInstance} for similarity.
	 *
	 * @param ri       the {@link IRecommendedInstance}
	 * @param instance the {@link IModelInstance}
	 * @return true, iff the {@link IRecommendedInstance} and {@link IModelInstance}
	 *         are similar.
	 */
	...
	/**
	 * Compares a given {@link IWord} with the type of a given
	 * {@link IModelInstance} for similarity.
	 *
	 * @param word     the {@link IWord}
	 * @param instance the {@link IModelInstance}
	 * @return true, iff the {@link IWord} and the type of the
	 *         {@link IModelInstance} are similar.
	 */
	...
	/**
	 * Checks the similarity of two string. Uses Jaro-Winkler similarity and
	 * Levenshtein to assess the similarity.
	 *
	...
	/**
	 * Checks the similarity of a list with test strings to a list of "original"
	 * strings. In this method all test strings are compared to all originals. For
	 * this the method uses the areWordsSimilar method with a given threshold. All
	 ...
	 * areWordsSimilar method with a given threshold.
	 *
	 * @param originals  list of original strings
	 * @param words2test list of test strings
	 * @return true if the list are similar, false if not
	 ...
	 * proportional threshold with the method areWordsOfListsSimilar method. If all
	 * lists are similar to the given instance by a threshold of 1-increase value
	 * the while loop can be left. If the while loop ends with more than one
	 * possibility or all remaining lists are sorted out in the same run, all are
	 * returned. Elsewhere only the remaining recommended instance is returned
	 * within the list.
	 *
	 ...
	 *         names)
	 */
	public static ImmutableList<IRecommendedInstance> getMostRecommendedInstancesToInstanceByReferences(IModelInstance instance, ImmutableList<IRecommendedInstance> recommendedInstances) {
	...
	private static boolean checkRecommendedInstanceForSelection(IModelInstance instance, IRecommendedInstance ri, double similarity) {
		...
		if (areWordsSimilar(instance.getFullName(), ri.getName()) || SimilarityUtils.areWordsOfListsSimilar(instanceNames, recommendedInstanceNameList, similarity)
		...
		for (var nounMapping : ri.getNameMappings()) {
			for (var surfaceForm : nounMapping.getSurfaceForms()) {
				...
				if (SimilarityUtils.areWordsOfListsSimilar(instanceNames, surfaceFormWords, similarity) || SimilarityUtils.areWordsOfListsSimilar(longestNameSplit, surfaceFormWords, similarity)) {
					return true;
				}
			}
		}
	...
	public static IPhraseMapping getMostSimilarPhraseMapping(IPhraseMapping phraseMappingX, ImmutableList<IPhraseMapping> phraseMappingYs, double minCosineSimilarity) {
		...
		for (IPhraseMapping phraseMappingY : phraseMappingYs) {
			double similarity = getPhraseMappingSimilarity(phraseMappingX, phraseMappingY, PhraseMappingAggregatorStrategy.MAX_SIMILARITY);
	...
	public enum PhraseMappingAggregatorStrategy implements ToDoubleBiFunction<IPhraseMapping, IPhraseMapping> {
		MAX_SIMILARITY(
				(a, b) -> uniqueDot(a.getPhrases(), b.getPhrases()).stream().mapToDouble(p -> cosineSimilarity(p.first().getPhraseVector(), p.second().getPhraseVector())).max().orElse(Double.NaN)),
		MIN_SIMILARITY(
				(a, b) -> uniqueDot(a.getPhrases(), b.getPhrases()).stream().mapToDouble(p -> cosineSimilarity(p.first().getPhraseVector(), p.second().getPhraseVector())).min().orElse(Double.NaN)),
		AVG_SIMILARITY((a, b) -> uniqueDot(a.getPhrases(), b.getPhrases()).stream().mapToDouble(p -> cosineSimilarity(p.first().getPhraseVector(), p.second().getPhraseVector())).average()
				.orElse(Double.NaN));

		private final ToDoubleBiFunction<IPhraseMapping, IPhraseMapping> mapper;

		PhraseMappingAggregatorStrategy(ToDoubleBiFunction<IPhraseMapping, IPhraseMapping> mapper) {
			this.mapper = mapper;
		}

		@Override
		public double applyAsDouble(IPhraseMapping phraseMapping, IPhraseMapping phraseMapping2) {
	...
	public static double getPhraseMappingSimilarity(IPhraseMapping phraseMappingX, IPhraseMapping phraseMappingY, PhraseMappingAggregatorStrategy strategy) {
		...
		if (coversOtherPhraseVector(phraseMappingX, phraseMappingY) || coversOtherPhraseVector(phraseMappingY, phraseMappingX)) {
			...
			if (containsAllNounMappingsOfPhraseMapping(phraseMappingX, phraseMappingY) && containsAllNounMappingsOfPhraseMapping(phraseMappingY, phraseMappingX)) {
				// TODO: HARD CODED
				return 1.0;
			}
		}
