...
public class TextStateWithoutClustering implements ITextState {
		...
		if (nounMappingsWithNode.isEmpty()) {
			...
			for (INounMapping nounMapping : nounMappingsWithNode) {
				...
				if (occurrences.stream().anyMatch(SimilarityUtils::containsSeparator) && !nounMapping.getReference().equalsIgnoreCase(ref)) {
					...
					List<INounMapping> nounMappingsWithRef = nounMappings.stream().filter(nm -> nm.getReference().equalsIgnoreCase(ref))
	...
	private boolean hasAnyTermMatchingKind(MappingKind kind, ITermMapping termWithMapping) {
		return termWithMapping.getMappings().stream()
	...
	public List<INounMapping> getNounMappingsWithSameReference(String ref) {
		return nounMappings.stream().filter(nMapping -> nMapping.getReference().equalsIgnoreCase(ref)).collect(Collectors.toList());
	}
