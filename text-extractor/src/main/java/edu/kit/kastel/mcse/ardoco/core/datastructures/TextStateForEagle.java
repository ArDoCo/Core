...
public class TextStateForEagle implements ITextStateWithDistributions {
	...
	private void addNounMapping(IWord word, String reference, MappingKind kind, double probability, List<String> occurrences) {
		...
		if (nounMappings.containsKey(reference)) {
			...
			List<String> similarRefs = nounMappings.keySet().stream().filter(ref -> SimilarityUtils.areWordsSimilar(ref, reference, similarityPercentage))
	...
	public void addNort(IWord n, String ref, double probability, List<String> occurrences) {
		...
		List<NounMappingForEagle> wordsWithSimilarNode = nounMappings.values().stream().filter(mapping -> mapping.getNodes().contains(n))
				.collect(Collectors.toList());
	...
	public List<INounMapping> getTypeNodesByNode(IWord node) {
		return nounMappings.values().stream().filter(n -> n.getNodes().contains(node)).filter(n -> n.getKind() == MappingKind.TYPE)
				.collect(Collectors.toList());
	}

	@Override
	public List<INounMapping> getNameNodesByNode(IWord node) {
		return nounMappings.values().stream().filter(n -> n.getNodes().contains(node)).filter(n -> n.getKind() == MappingKind.NAME)
				.collect(Collectors.toList());
	}

	@Override
	public List<INounMapping> getNortNodesByNode(IWord node) {
		return nounMappings.values().stream().filter(n -> n.getNodes().contains(node)).filter(n -> n.getKind() == MappingKind.NAME_OR_TYPE)
				.collect(Collectors.toList());
	}
	...
	public boolean isNodeContainedByNameOrTypeNodes(IWord node) {
		return !nounMappings.values().stream().filter(n -> n.getKind().equals(MappingKind.NAME_OR_TYPE)).filter(n -> n.getNodes().contains(node)).findAny()
				.isEmpty();
	}
