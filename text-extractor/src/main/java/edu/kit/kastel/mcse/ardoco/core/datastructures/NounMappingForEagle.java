...
public class NounMappingForEagle implements INounMappingWithDistribution {
	 ...
	 * {
	 *
	 * double name = distribution.get(MappingKind.NAME); double type =
	 * distribution.get(MappingKind.TYPE); double nort =
	 * distribution.get(MappingKind.NAME_OR_TYPE);
	 *
	 * double loss = 0.8; double currentProbability = distribution.get(kind);
	 *
	 *
	 * if (kind.equals(MappingKind.NAME_OR_TYPE) && (name > 0 || type > 0)) { double
	 * probabilityToSetName = loss * ;
	 *
	 *
	 *
	 *
	 * } else { double probabilityToSet = loss * (newProbability +
	 * currentProbability);
	 *
	 * if (name + type + nort + probabilityToSet - distribution.get(kind) <= 1) {
	 * distribution.put(kind, probabilityToSet); } else { double scale = 1 / (name +
	 * type + nort - distribution.get(kind) + probabilityToSet);
