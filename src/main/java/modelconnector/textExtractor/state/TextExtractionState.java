...
public class TextExtractionState {
     ...
     *
     * @param n
     *            node to add to the mappings
     * @param ref
     *            reference to add to the mappings
     * @param occurrences
     *            occurrences to add to the mappings
     * @param kind
     *            kind of the mapping
     */
    private void hardAdd(INode n, String ref, List<String> occurrences, double probability, MappingKind kind) {

        double hardAddProbability = ModelConnectorConfiguration.TEXT_EXTRACTION_STATE_HARD_ADD_PROBABILITY
                * probability;
        List<NounMapping> nounMappingsWithNode = getNounMappingsByNode(n);
    ...
    private void addOrUpdate(List<NounMapping> nounMappingsWithRef, INode n, String ref, List<String> occurrences,
            double hardAddProbability, MappingKind kind) {
        if (nounMappingsWithRef.isEmpty()) {

            List<NounMapping> nounMappingsWithOcc = nounMappings.stream()
                                                                .filter(//
                                                                        nsn -> SimilarityUtils.areWordsOfListsSimilar(
                                                                                nsn.getRepresentativeComparables(),
                                                                                occurrences))
                                                                .collect(Collectors.toList());

            NounMapping createdMapping = createMappingTypeDependentMapping(kind, n, hardAddProbability, ref,
                    occurrences);
     ...
     * @param mapping
     */
    private void extendNounMappingsWithOccurrences(List<NounMapping> nounMappingsWithOccurrences, String ref,
            NounMapping mapping) {

        for (NounMapping nnm : nounMappingsWithOccurrences) {
            List<String> occSimilar = nnm.getOccurrences()
                                         .stream()
                                         .filter(occ -> SimilarityUtils.areWordsSimilar(ref, occ))
                                         .collect(Collectors.toList());
            occSimilar.stream()
                      .forEach(occ -> nnm.copyOccurrencesAndNodesTo(occ, mapping));
        }
    }
     ...
     * @param occurrences
     *            the occurrences to update with
     * @param kind
     *            the kind of the updating mapping
     * @param probability
     *            the probability of the updating mapping
     */
    private void updateKindProbOccOfMapping(NounMapping nounMapping, List<String> occurrences, MappingKind kind,
            Double probability) {
    ...
    private void updateKindOfNounMapping(NounMapping nounMapping, MappingKind kind, double probability) {
        ...
        List<TermMapping> termsWithMapping = terms.stream()
                                                  .filter(//
                                                          t -> t.getMappings()
        ...
        for (TermMapping termWithMapping : termsWithMapping) {
            if (!termWithMapping.getKind()
                                .equals(kind)
                    && hasAnyTermMatchingKind(kind, termWithMapping)) {
    ...
    private boolean hasAnyTermMatchingKind(MappingKind kind, TermMapping termWithMapping) {
        return termWithMapping.getMappings()
                              .stream()
                              .anyMatch(mapping -> !mapping.getKind()
                                                           .equals(kind)
                                      && !mapping.getKind()
                                                 .equals(MappingKind.NAME_OR_TYPE));
    }
     ...
     *
     * @param nounMapping
     *            the noun mapping to update
     * @param mt
     *            the kind to update with
     * @param n
     ...
     *            the occurrences to update with
     */
    private void extendMappingBy(NounMapping nounMapping, MappingKind mt, INode n, Double probability,
            List<String> occurrences) {

        if (!nounMapping.getNodes()
                        .contains(n)) {
     ...
     * @param occurrences
     *            the occurrences for the mapping
     * @return the created noun mapping
     */
    private NounMapping createMappingTypeDependentMapping(MappingKind kind, INode n, double probability, String ref,
            List<String> occurrences) {
     ...
     * removed and the method {@link #hardAdd(INode, String, List, MappingKind)} is called and returned. If neither the
     ...
     * @param occurrences
     *            appearances of the mapping to add
     */
    private void addNounMapping(INode node, String reference, MappingKind kind, double probability,
            List<String> occurrences) {
        ...
        List<NounMapping> nounMappingsWithNode = getNounMappingsByNode(node);
        List<NounMapping> nounMappingsWithRef = SimilarityUtils.getMostLikelyNMappingsByReference(reference,
                nounMappings);

        if (nounMappingsWithNode.isEmpty() && nounMappingsWithRef.isEmpty()) {
            NounMapping createdMapping = NounMapping.createMappingTypeNode(node, reference, kind, probability,
                    occurrences);
            nounMappings.add(createdMapping);
    ...
    private void updateMapping(NounMapping nnm, MappingKind kind, double probability, List<String> occurrences) {
        if (kind.equals(MappingKind.NAME_OR_TYPE)) {
            ...
            boolean existingMappingIsNortAndNewProbabilityIsHigher = nnm.getKind()
                                                                        .equals(MappingKind.NAME_OR_TYPE)
                    || probability >= nnm.getProbability();
     ...
     *            node of the mapping
     * @param name
     *            reference of the mapping
     * @param probability
     *            probability to be a name mapping
     * @param occurrences
     *            list of the appearances of the mapping
     */
     ...
     *            reference of the mapping
     * @param probability
     *            probability to be a name or type mapping
     */
     ...
     *            reference of the mapping
     * @param probability
     *            probability to be a type mapping
     */
     ...
     *
     * @param node1
     *            first relation end point
     * @param node2
     *            second relation end point
     * @param probability
     *            probability of being a relation
     ...
     * @param mapping1
     *            the first mapping of the term
     * @param mapping2
     *            the second mapping of the term
     * @param kind
     *            the kind of the term
     * @param probability
     *            the probability that this term is from that kind
     */
    public void addTerm(String reference, NounMapping mapping1, NounMapping mapping2, MappingKind kind,
            double probability) {
     ...
     *            the second mapping of the term
     * @param otherMappings
     *            other mappings of the term
     * @param kind
     *            the kind of the term
     * @param probability
     *            the probability that this term is from that kind
     */
    public void addTerm(String reference, NounMapping mapping1, NounMapping mapping2, List<NounMapping> otherMappings,
     ...
     *            the second mapping of the term
     * @param probability
     *            the probability that this term is a name
     */
     ...
     *            the second mapping of the term
     * @param probability
     *            the probability that this term is a type
     */
    ...
    private void addTerm(String reference, List<NounMapping> mappings, MappingKind kind, double probability) {
        ...
        if (!includedTerms.isEmpty()) {
            ...
            if (mappings.size() <= 2) {
                ...
                term = new TermMapping(reference, mappings.get(0), mappings.get(1),
                        mappings.subList(2, mappings.size() - 1), kind, probability);
            }
            terms.add(term);
        }
    }
     ...
     *
     * @param nounMappings
     *            the nounMappings to search for
    ...
    public List<TermMapping> getTermsByMappings(List<NounMapping> nounMappings) {
        return terms.stream()
                    .filter(//
                            t -> t.getMappings()
                                  .containsAll(nounMappings) && nounMappings.containsAll(t.getMappings()))
                    .//
                    collect(Collectors.toList());
    }
    ...
    public List<TermMapping> getTermsBySimilarReference(String reference) {
        return terms.stream()
                    .filter(t -> SimilarityUtils.areWordsSimilar(reference, t.getReference()))
                    .collect(Collectors.toList());
    }
     ...
     *            the nounMappings to search for
     * @param kind
     *            the kind of the term mappings to search for
    ...
    public List<NounMapping> getTypes() {
        return nounMappings.stream()
                           .filter(n -> n.getKind()
                                         .equals(MappingKind.TYPE))
                           .collect(Collectors.toList());
    }
    ...
    public List<NounMapping> getNounMappingsByNode(INode n) {
        return nounMappings.stream()
                           .filter(nMapping -> nMapping.getNodes()
                                                       .contains(n))
                           .collect(Collectors.toList());
    }
    ...
    public List<NounMapping> getNounMappingsWithSameReference(String ref) {
        return nounMappings.stream()
                           .filter(nMapping -> nMapping.getReference()
                                                       .contentEquals(ref))
                           .collect(Collectors.toList());
    }
    ...
    public List<String> getNameList() {
        ...
        List<NounMapping> nameMappings = getNames();
    ...
    public List<String> getNameTermList() {
        Set<String> names = new HashSet<>();
        List<TermMapping> nameMappings = getNameTerms();
    ...
    public List<String> getNortList() {
        ...
        List<NounMapping> nortMappings = getNameOrTypeMappings();
    ...
    public List<String> getTypeList() {
        ...
        List<NounMapping> typeMappings = getTypes();
    ...
    public List<String> getTypeTermList() {
        ...
        Set<String> types = new HashSet<>();
        List<TermMapping> typeMappings = getTypeTerms();
     ...
     *
     * @param nounMapping
     *            the noun mapping that should be contained.
