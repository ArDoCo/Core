...
public class ConnectionState {
	...
	public List<InstanceLink> getInstanceLinksByRecommendedInstance(RecommendedInstance recommendedInstance) {
		return this.instanceLinks.stream().filter(il -> il.getTextualInstance().equals(recommendedInstance)).collect(Collectors.toList());
	}
	...
	public void addToLinks(RecommendedInstance recommendedModelInstance, Instance instance, double probability) {
		...
		if (!this.isContainedByInstanceLinks(instancelink)) {
			...
			InstanceLink instanceLink = this.instanceLinks.stream().filter(il -> il.equals(instancelink)).findFirst().get();
			instanceLink.getTextualInstance().addMappings(instancelink.getTextualInstance().getNameMappings(), instancelink.getTextualInstance().getTypeMappings());
		}
	}
	...
	public void addToLinks(RecommendedRelation recommendedModelRelation, Relation relation, double probability) {
		...
		if (!this.isContainedByRelationLinks(rel)) {
			relationLinks.add(rel);
		} else {
			RelationLink rl = this.relationLinks.stream().filter(rela -> rela.equals(rel)).findFirst().get();
			rl.getTextualRelation().addOccurrences(recommendedModelRelation.getNodes());
		}
	}
