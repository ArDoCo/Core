...
public class ExtractedTermsAnalyzer extends RecommendationAnalyzer {
     ...
     *
     * @param graph
     *            the PARSE graph to work with
     * @param textExtractionState
     *            the text extraction state to work with
     * @param modelExtractionState
     *            the model extractin state to work with
     * @param recommendationState
     *            the recommendation state to write the results and read existing recommendations from
     */
    public ExtractedTermsAnalyzer(//
        ...
        super(DependencyType.TEXT_RECOMMENDATION, graph, textExtractionState, modelExtractionState,
                recommendationState);
    }
    ...
    private void createRecommendedInstancesForTerm(INode node) {
        ...
        List<TermMapping> termMappings = textExtractionState.getTermMappingsByNode(node);
        ...
        for (TermMapping term : termMappings) {
            ...
            if (adjacentNounMappings.isEmpty() && term.getKind()
                                                      .equals(MappingKind.NAME)) {

                recommendationState.addRecommendedInstanceJustName(term.getReference(), probabilityJustName,
                        term.getMappings());
        ...
        for (TermMapping term : termMappings) {
            ...
            for (NounMapping wordMapping : wordMappings) {
                ...
                for (int i = position - 1; i >= 0 && !stop; i--) {
                    ...
                    String reference = termNounMappings.get(i)
                                                       .getReference();
                    // TODO: check this!
    ...
    private void createRecommendedInstancesForAdjacentTermMappings(INode termStartNode, TermMapping term,
            List<TermMapping> adjacentTermMappings) {
    ...
    private List<TermMapping> getCompletePreAdjTermMappings(List<TermMapping> possibleTermMappings,
            INode termStartNode) {
        String sentence = termStartNode.getAttributeValue(SENTENCE_NUMBER)
                                       .toString();
        INode preTermNode = getPreTermNode(termStartNode);
        ...
        for (TermMapping adjTerm : possibleTermMappings) {
            ...
            while (nounMappings.isEmpty()) {
                ...
                // TODO: check this!
                if (sentence.contentEquals(preTermNode.getAttributeValue(SENTENCE_NUMBER)
                                                      .toString())) {
                    break;
                }
    ...
    private List<TermMapping> getCompleteAfterAdjTermMappings(List<TermMapping> possibleTermMappings,
            INode termStartNode, TermMapping term) {
        ...
        for (TermMapping adjTerm : possibleTermMappings) {
            ...
            while (nounMappings.isEmpty()) {
                ...
                if (sentence.contentEquals(afterTermNode.getAttributeValue(SENTENCE_NUMBER)
                                                        .toString())) {
                    break;
                }
    ...
    private INode getPreTermNode(INode termStartNode) {
    ...
    private INode getAfterTermNode(INode termStartNode, TermMapping term) {
        ...
        for (NounMapping element : term.getMappings()) { // TODO check!
            afterTermNode = GraphUtils.getNextNode(afterTermNode, relArcType);
        }
    ...
    private List<NounMapping> getTermAdjacentNounMappings(INode node, TermMapping term) {
        ...
        INode preTermNode = getPreTermNode(node);
        INode afterTermNode = getAfterTermNode(node, term);
        String sentence = node.getAttributeValue(SENTENCE_NUMBER)
                              .toString();
        ...
        if (sentence.contentEquals(preTermNode.getAttributeValue(SENTENCE_NUMBER)
            ...
            List<NounMapping> nounMappingsOfPreTermNode = textExtractionState.getNounMappingsByNode(preTermNode);
            possibleMappings.addAll(nounMappingsOfPreTermNode);
        }
        if (sentence.contentEquals(afterTermNode.getAttributeValue(SENTENCE_NUMBER)
                                                .toString())) {
            List<NounMapping> nounMappingsOfAfterTermNode = textExtractionState.getNounMappingsByNode(afterTermNode);
            possibleMappings.addAll(nounMappingsOfAfterTermNode);
        }
        possibleMappings = possibleMappings.stream()
                                           .filter(nounMapping -> !nounMapping.getKind()
                                                                              .equals(kind))
    ...
    private void createRecommendedInstancesOfAdjacentTerms(TermMapping term, List<TermMapping> adjacentTerms) {
        ...
        if (kind.equals(MappingKind.NAME) && !adjacentTerms.isEmpty()) {
            for (TermMapping adjTerm : adjacentTerms) {
                recommendationState.addRecommendedInstance(term.getReference(), adjTerm.getReference(),
                        probabilityAdjacentTerm, term.getMappings(), adjTerm.getMappings());

            }
        } else if (kind.equals(MappingKind.TYPE) && !adjacentTerms.isEmpty()) {
            for (TermMapping adjTerm : adjacentTerms) {
                recommendationState.addRecommendedInstance(adjTerm.getReference(), term.getReference(),
                        probabilityAdjacentTerm, adjTerm.getMappings(), term.getMappings());
            }

        }
    }

    private void createRecommendedInstancesForSurroundingNounMappings(TermMapping term,
            List<NounMapping> surroundingNounMappings) {
        MappingKind kind = term.getKind();

        if (kind.equals(MappingKind.NAME) && !surroundingNounMappings.isEmpty()) {
            for (NounMapping nounMapping : surroundingNounMappings) {

                recommendationState.addRecommendedInstance(term.getReference(), nounMapping.getReference(),
                        probabilityJustAdjacentNoun, term.getMappings(), List.of(nounMapping));
            }
        } else if (kind.equals(MappingKind.TYPE) && !surroundingNounMappings.isEmpty()) {
            for (NounMapping nounMapping : surroundingNounMappings) {

                recommendationState.addRecommendedInstance(nounMapping.getReference(), term.getReference(),
                        probabilityJustAdjacentNoun, List.of(nounMapping), term.getMappings());
            }
        }
