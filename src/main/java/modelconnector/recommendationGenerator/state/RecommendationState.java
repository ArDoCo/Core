...
public class RecommendationState {
     ...
     *
     * @param name
     *            name of that recommended relation
     * @param probability
     *            probability of being in the model
     * @param ri1
     *            first end point of the relation as recommended instance
     * @param ri2
     *            second end point of the relation as recommended instance
     * @param otherInstances
     *            other involved recommended instances
     * @param occurrences
     *            nodes representing the relation
     */
    public void addRecommendedRelation(String name, RecommendedInstance ri1, RecommendedInstance ri2,
            List<RecommendedInstance> otherInstances, double probability, List<INode> occurrences) {
        ...
        Optional<RecommendedRelation> recr = recommendedRelations.stream()
                                                                 .filter(//
                                                                         r -> r.getRelationInstances()
                                                                               .containsAll(ris)
                                                                                 && ris.containsAll(
                                                                                         r.getRelationInstances()))
                                                                 .findAny();
     ...
     *
     * @param name
     *            name of that recommended instance
     * @param probability
     *            probability of being in the model
     * @param nameMappings
     *            name mappings representing that recommended instance
     */
     ...
     *            name of that recommended instance
     * @param type
     *            type of that recommended instance
     * @param probability
     *            probability of being in the model
     * @param nameMappings
     *            name mappings representing the name of the recommended instance
     * @param typeMappings
     *            type mappings representing the type of the recommended instance
     * @return the added recommended instance
     */
    public RecommendedInstance addRecommendedInstance(String name, String type, double probability,
            List<NounMapping> nameMappings, List<NounMapping> typeMappings) {

        RecommendedInstance ri = new RecommendedInstance(name, type, probability,
                new ArrayList<>(new HashSet<>(nameMappings)), new ArrayList<>(new HashSet<>(typeMappings)));
    ...
    private void addRecommendedInstance(RecommendedInstance ri) {
        ...
        List<RecommendedInstance> risWithExactName = recommendedInstances.stream()
                                                                         .filter(r -> r.getName()
                                                                                       .contentEquals(ri.getName()))
                                                                         .collect(Collectors.toList());
        List<RecommendedInstance> risWithExactNameAndType = risWithExactName.stream()
                                                                            .filter(r -> r.getType()
                                                                                          .contentEquals(ri.getType()))
                                                                            .collect(Collectors.toList());
        ...
        if (risWithExactNameAndType.isEmpty()) {
            ...
            risWithExactNameAndType.get(0)
                                   .addMappings(ri.getNameMappings(), ri.getTypeMappings());

        }

    }

    private void processRecommendedInstancesWithNoExactNameAndType(RecommendedInstance ri,
            List<RecommendedInstance> risWithExactName) {
    ...
    private boolean recommendedInstancesHasEmptyType(RecommendedInstance ri, RecommendedInstance riWithExactName) {
        return riWithExactName.getType()
                              .contentEquals("")
                && !ri.getType()
                      .contentEquals("");
    }
     ...
     *
     * @param mapping
     *            given mapping to search for in types
    ...
    public List<RecommendedInstance> getRecommendedInstancesByTypeMapping(NounMapping mapping) {
        return recommendedInstances.stream()
                                   .filter(sinstance -> sinstance.getTypeMappings()
                                                                 .contains(mapping))
                                   .collect(Collectors.toList());
    }
     ...
     *
     * @param name
     *            given name to search for in names
     ...
     *
     * @param type
     *            given type to search for in types
    ...
    public List<RecommendedInstance> getRecommendedInstancesBySimilarType(String type) {
        return recommendedInstances.stream()
                                   .filter(ri -> SimilarityUtils.areWordsSimilar(ri.getType(), type))
                                   .collect(Collectors.toList());
    }
