...
/**
 * This helper class encapsulates multiple methods, that are used to move on a
 * graph or access node entries.
 *
 * @author Sophie
 *
 */
public final class GraphUtils {
	 ...
	 *
	 * @param n          node
	 * @param relArcType the arctype of the edge to follow
	...
	public static boolean checkIfPrevNodeIsDt(INode n, IArcType relArcType) {
		...
		if (prevNode != null && prevNode.getAttributeValue("pos").toString().equals("DT")) {
			return true;
		}
		return false;
	}
	...
	public static boolean checkForIndirectPrevDt(INode n, IArcType relArcType) {
		...
		if (checkIfPrevNodeIsDt(n, relArcType)) {
			String prevVal = getNodeValue(getPreviousNode(n, relArcType));
			if (prevVal.equalsIgnoreCase("a") || prevVal.equalsIgnoreCase("an")) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Returns the previous node. The previous node is the source node of the
	 * incoming rel-Arc.
	 *
	 * @param n          node
	 * @param relArcType the arctype of the edge to follow
	...
	public static INode getPreviousNode(INode n, IArcType relArcType) {
		...
		Optional<? extends IArc> inNextArc = inRelArcs.stream().filter(a -> a.getAttributeValue("value").toString().equals("NEXT")).findFirst();

		if (!inNextArc.isEmpty()) {
			return inNextArc.get().getSourceNode();
		}
		return null;
	}
	...
	private static String getNodePosition(INode n) {
		ArrayList<Pair<String, Object>> attributeNameValuePairs = n.getAllAttributeNamesAndValuesAsPair();
		for (Pair<String, Object> pair : attributeNameValuePairs) {
			if (pair.getLeft().equals("position")) {
				return pair.getRight().toString();
			}
		}
		return null;
	 ...
	 * @param relArcType has to be the relation arc type of PARSE
	...
	public static INode getNextNode(INode n, IArcType relArcType) {
		...
		Optional<? extends IArc> outNextArc = outRelArcs.stream().filter(a -> a.getAttributeValue("value").toString().equals("NEXT")).findFirst();

		if (!outNextArc.isEmpty()) {
			return outNextArc.get().getTargetNode();
		}
		return n;
	}
