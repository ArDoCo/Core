...
public final class SimilarityUtils {
     ...
     *            original string
     * @param word2test
     *            test string to match the original
     * @param threshold
     *            threshold for granularity of similarity
    ...
    public static boolean areWordsSimilar(String original, String word2test, Double threshold) {
        if (original.toLowerCase()
                    .split(" ").length != word2test.toLowerCase()
                                                   .split(" ").length) {
            return false;
        }
        ...
        if (original.length() <= areWordsSimilarMinLength) {
            ...
            if (ldist <= areWordsSimilarMaxLdist || lcscount >= (int) (original.length() * threshold)) {
                return true;
            }
        }
     ...
     * @param minProportion
     *            threshold for proportional similarity between the lists
        ...
        for (String o : originals) {
            for (String wd : words2test) {
                if (areWordsSimilar(o, wd,
                        ModelConnectorConfiguration.ARE_WORDS_OF_LISTS_SIMILAR_WORD_SIMILARITY_THRESHOLD)) {
                    counter++;
                }
            }
        }
     ...
     */
    public static boolean areWordsOfListsSimilar(List<String> originals, List<String> words2test) {
        return areWordsOfListsSimilar(originals, words2test,
                ModelConnectorConfiguration.ARE_WORDS_OF_LISTS_SIMILAR_DEFAULT_THRESHOLD);
    }
     ...
     */
    public static List<NounMapping> getAllSimilarNMappingToInstanceByReferences(Instance instance,
            List<NounMapping> mapping) {
        List<String> instanceNames = instance.getNames();
        return mapping.stream()
                      .filter(n -> SimilarityUtils.areWordsOfListsSimilar(instanceNames, List.of(n.getReference())))
    ...
    public static List<NounMapping> getAllSimilarNMappingsByReference(String ref, List<NounMapping> nounMappings) {
        ...
        return nounMappings.stream()
                           .filter(n -> SimilarityUtils.areWordsSimilar(n.getReference(), ref))
                           .collect(Collectors.toList());
     ...
     *
     * @param instance
     *            instance to use as original for compare
     * @param recommendedInstances
     *            recommended instances to check for similarity
     * @return a list of the most similar recommended instances (to the instance names)
     */
    public static List<RecommendedInstance> getMostRecommendedInstancesToInstanceByReferences(Instance instance,
            List<RecommendedInstance> recommendedInstances) {
        List<String> instanceNames = instance.getNames();
        List<RecommendedInstance> selection = recommendedInstances.stream()
                                                                  .filter(//
                                                                          ri -> SimilarityUtils.areWordsOfListsSimilar(
                                                                                  instanceNames, List.of(ri.getName())))
                                                                  .collect(Collectors.toList());
        ...
        while (whileSelection.size() > 1 && getMostRecommendedIByRefMinProportion <= 1) {
            ...
            List<RecommendedInstance> risToRemove = new ArrayList<>();
            for (RecommendedInstance ri : whileSelection) {

                if (areWordsSimilar(String.join(" ", instanceNames), String.join(" ", ri.getName()),
                        1 - getMostRecommendedIByRefIncrease)) {
                    allListsSimilar++;
                }

                if (!SimilarityUtils.areWordsOfListsSimilar(instanceNames, List.of(ri.getName()),
                        getMostRecommendedIByRefMinProportion)) {
                    risToRemove.add(ri);
                }
            }
    ...
    public static List<NounMapping> getMostLikelyNMappingsByReference(String ref, List<NounMapping> nounMappings) {
        ...
        List<NounMapping> selection = new ArrayList<>(
                SimilarityUtils.getAllSimilarNMappingsByReference(ref, nounMappings));
        ...
        while (whileSelection.size() > 1 && threshold < 1) {
            ...
            whileSelection = whileSelection.stream()
                                           .filter(nnm -> SimilarityUtils.areWordsSimilar(ref, nnm.getReference(), wTh))
                                           .collect(Collectors.toList());
     ...
     *
     * @param ri
     *            recommended instance under investigation
     * @return most similar mappings
     */
    public static List<NounMapping> getMostLikelyNNMappingsByName(RecommendedInstance ri) {
