 ...
 */
@Deterministic public class OrderedMatchingFilter<A, B> implements MatchingFilter<A, B> {
    ...
    private static <A, B> void selectBestMappings(List<Assignment<A, B>> sortedAssignments, Set<A> usedElementsInA,
            Set<B> usedElementsInB, MutableBiMap<A, B> filteredPairs) {
        for (Assignment<A, B> assignment : sortedAssignments) {
            if (!usedElementsInA.contains(assignment.pair()
                    .getFirst()) && !usedElementsInB.contains(assignment.pair()
                    .getSecond())) {
                filteredPairs.put(assignment.pair()
                        .getFirst(), assignment.pair()
                        .getSecond());
                usedElementsInA.add(assignment.pair()
                        .getFirst());
                usedElementsInB.add(assignment.pair()
                        .getSecond());
            }
        }
    }
    ...
    public MutableBiMap<A, B> filter(SimilarityMapping<A, B> finalMapping, SimilarityMapping<A, B> initialMapping) {
        ...
        List<Assignment<A, B>> sortedAssignments = initialMapping == null
                ? this.sortMappings(finalMapping)
                : this.sortMappings(finalMapping, initialMapping);
    ...
    private List<Assignment<A, B>> sortMappings(SimilarityMapping<A, B> finalMapping,
            SimilarityMapping<A, B> initialMapping) {
