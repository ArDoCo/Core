...
/**
 * Provides utility methods that are shared by
 * {@link edu.kit.kastel.mcse.ardoco.core.pipeline.agent.Informant informants} of this stage.
 */
public class DiagramUtil {
  ...
  /**
   * Returns the map containing the highest similarity of each word contained by the recommended
   * instance to any word contained by the box.
   *
   ...
   */
  public static @NotNull Map<Word, Double> calculateHighestSimilarity(@NotNull Box box,
                                                                      @NotNull RecommendedInstance recommendedInstance) {
    var deNames = box.getReferences();
    var words =
            recommendedInstance.getNameMappings().stream().flatMap(nm -> nm.getWords().stream()).collect(Collectors.toSet());
    return calculateHighestSimilarity(words, deNames);
  }

  /**
   * {@return the map containing the highest similarity of each word to any word from the target
   * words}
   *
   * @param words   the words
   * @param targets the target words
   */
  public static @NotNull Map<Word, Double> calculateHighestSimilarity(@NotNull Set<Word> words,
                                                                      @NotNull Set<String> targets) {
    return words.stream().collect(Collectors.toMap(w -> w, w -> calculateHighestSimilarity(w,
            targets)));
  }
   ...
   * @param targets the target words
   */
  public static double calculateHighestSimilarity(@NotNull Word word,
                                                  @NotNull Set<String> targets) {
    return targets.stream()
            .map(name -> WordSimUtils.getSimilarity(word.getText(), name,
                    SimilarityStrategy.MAXIMUM, true))
  ...
  public static double calculateSimilarity(@NotNull NounMapping nounMapping, @NotNull Box box) {
    ...
    return nounMapping.getReferenceWords().stream().map(word -> calculateHighestSimilarity(word,
            targets)).max(Double::compareTo).orElse(0.0);
  }
  ...
  /**
   * {@return a set of alternative texts extracted from the input text}. The text is processed
   * with {@link #splitBracketsAndEnumerations(String)} and
  ...
  /**
   * Splits the string around brackets and commas. The results are trimmed. <span style="
   * white-space: nowrap;">Example: "Lorem (ipsum), Dolor, sit (Amet)" ->
  ...
  /**
   * Decamels the word and returns it as words joined by space. <span style=" white-space:
   * nowrap;">Example: "CamelCaseExample" -> "Camel Case Example",
  ...
  private static @NotNull String getDeCameledText(@NotNull String word) {
    return String.join(" ", word.split("(?<!([A-Z]))(?=[A-Z])|(?<!^)(?=[A-Z][a-z])")).replaceAll(
            "\\s+", " ");
  }

  /**
   * Determines a set of possible references for a textBox. Tries to filter out technical terms
   * using {@link DbPediaHelper}.
   *
  ...
  public static @NotNull Set<String> getReferences(@NotNull TextBox textBox,
                                                   ImmutableMap<String, Disambiguation> disambiguations) {
    ...
    for (var meanings : crossProduct) {
      ...
      for (Pair<String, String> replacement : meanings) {
        textWithReplacements = textWithReplacements.replace(replacement.first(),
                replacement.second());
      }
