 ...
 */
@Deterministic public class NameExtension<B, E> extends Inconsistency<B, E> {
    ...
    public static <B, E> List<Inconsistency<B, E>> discover(List<Inconsistency<B, E>> inconsistencies,
            Function<B, String> boxNameProvider, Function<E, String> entityNameProvider) {
        ...
        for (var nameInconsistency : nameInconsistencies) {
            ...
            if (nameInconsistency.getActualName()
                    .contains(nameInconsistency.getExpectedName())) {
                newInconsistencies.add(new NameExtension<>(nameInconsistency.getBox(), nameInconsistency.getEntity(),
                        nameInconsistency.getExpectedName()));
        ...
        for (var missingBoxInconsistency : missingBoxInconsistencies) {
            for (var unexpectedBoxInconsistency : unexpectedBoxInconsistencies) {
                ...
                if (unexpectedName.contains(missingName)) {
                    newInconsistencies.add(new NameExtension<>(unexpectedBoxInconsistency.getBox(),
                            missingBoxInconsistency.getEntity(), missingName));
    ...
    private static <B, E> List<Inconsistency<B, E>> getAllMissingBoxInconsistencies(
            Set<Inconsistency<B, E>> inconsistencies) {
        return inconsistencies.stream()
                .filter(inconsistency -> inconsistency.getType() == InconsistencyType.MISSING_BOX)
                .toList();
    }

    private static <B, E> List<Inconsistency<B, E>> getAllUnexpectedBoxInconsistencies(
            Set<Inconsistency<B, E>> inconsistencies) {
        return inconsistencies.stream()
                .filter(inconsistency -> inconsistency.getType() == InconsistencyType.UNEXPECTED_BOX)
                .toList();
    }

    @Override
    public <R, M> Inconsistency<R, M> map(Function<B, R> boxMapper, Function<E, M> entityMapper) {
        return new NameExtension<>(boxMapper.apply(this.getBox()), entityMapper.apply(this.getEntity()),
                this.expectedName);
    }
    ...
    public String toString() {
        return new ToStringBuilder(this).append("expectedName", this.expectedName)
                .toString();
    }
}
