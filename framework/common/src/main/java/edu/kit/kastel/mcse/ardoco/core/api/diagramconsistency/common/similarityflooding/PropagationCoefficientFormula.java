 ...
 */
@FunctionalInterface
@Deterministic public interface PropagationCoefficientFormula<A, B> {
    /**
    ...
    static <A, B> PropagationCoefficientFormula<A, B> getInverseAverageFormula() {
        return vertices -> 2.0 / (vertices.getFirst()
                .size() + vertices.getSecond()
                .size());
    }
    ...
    static <A, B> PropagationCoefficientFormula<A, B> getInverseProductFormula() {
        return vertices -> {
            if (vertices.getFirst()
                    .isEmpty() || vertices.getSecond()
                    .isEmpty()) {
                return 0.0;
            }

            return 1.0 / (vertices.getFirst()
                    .size() * vertices.getSecond()
                    .size());
        };
    }
