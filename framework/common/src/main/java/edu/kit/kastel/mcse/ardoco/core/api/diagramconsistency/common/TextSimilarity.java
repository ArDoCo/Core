 ...
 */
@Deterministic public final class TextSimilarity {
    ...
    public static double byLevenshtein(String textA, String textB) {
        return CACHE_FOR_BASIC_FUNCTION.computeIfAbsent(new UnorderedPair<>(textA, textB), pair -> {
            ...
            double maxDistance = Math.max(pair.getFirst()
    ...
    public static double byLevenshteinCaseInsensitive(String textA, String textB) {
        return CACHE_FOR_CASE_INSENSITIVE_FUNCTION.computeIfAbsent(new UnorderedPair<>(textA, textB), pair -> {
            ...
            double maxDistance = Math.max(pair.getFirst()
                    .length(), pair.getSecond()
                    .length());
            double levenshteinDistance = algorithm.apply(pair.getFirst()
                    .toLowerCase(), pair.getSecond()
                    .toLowerCase());
    ...
    public static double byJaroWinkler(String textA, String textB) {
        return CACHE_FOR_JARO_WINKLER_FUNCTION.computeIfAbsent(new UnorderedPair<>(textA, textB),
                pair -> JARO_WINKLER_SIMILARITY.apply(pair.getFirst()
                        .toLowerCase(), pair.getSecond()
                        .toLowerCase()));
    }
    ...
    public static double byJaccard(String textA, String textB) {
        return CACHE_FOR_JACCARD_FUNCTION.computeIfAbsent(new UnorderedPair<>(textA, textB),
                pair -> JACCARD_SIMILARITY.apply(pair.getFirst()
                        .toLowerCase(), pair.getSecond()
                        .toLowerCase()));
    }
}
