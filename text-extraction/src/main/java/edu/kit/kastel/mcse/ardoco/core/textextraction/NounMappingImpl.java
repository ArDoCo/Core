...
public class NounMappingImpl implements NounMapping {
	 ...
	 */
	public NounMappingImpl(ImmutableList<Word> words, Map<MappingKind, Confidence> distribution, ImmutableList<Word> referenceWords, ImmutableList<String> surfaceForms) {
	 ...
	 * Instantiates a new noun mapping.
	 */
	public NounMappingImpl(ImmutableList<Word> words, MappingKind kind, Claimant claimant, double probability, ImmutableList<Word> referenceWords, ImmutableList<String> occurrences) {
	...
	public NounMapping createCopy() {
		return new NounMappingImpl(words.toImmutable(), JavaUtils.copyMap(this.distribution, Confidence::createCopy), Lists.immutable.withAll(referenceWords), surfaceForms.toImmutable());
	}
	...
	public NounMapping merge(NounMapping other) {
		...
		otherDistribution.keySet().forEach(kind -> Confidence.merge(distribution.get(kind), otherDistribution.get(kind), DEFAULT_AGGREGATOR, DEFAULT_AGGREGATOR));
	...
	public String toString() {
		return "NounMapping [" + "distribution=" + distribution.entrySet().stream().map(entry -> entry.getKey() + ":" + entry.getValue()).collect(Collectors.joining(",")) + //
	...
	public boolean isTheSameAs(NounMapping other) {
		return Objects.equals(getReference(), other.getReference()) && Objects.equals(getWords(), other.getWords()) && Objects.equals(getKind(), other.getKind()) && Objects.equals(getPhrases(),
				other.getPhrases());
	}
