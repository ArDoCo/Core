...
public class TextState extends AbstractState implements ITextState {
	...
	public ImmutableList<IPhraseMapping> getPhraseMappingsByNounMapping(INounMapping nm) {
		...
		/*
		 * if (nm.getPhrases().size() != result.get(0).getPhrases().size() ||
		 * nm.getPhrases().select(p -> !result.get(0).getPhrases().contains(p)).size() >
		 * 0) { int i = 0; }
		 */
	...
	public void addNounMapping(INounMapping nounMapping, IClaimant claimant) {
		addNounMappingOrExtendExistingNounMapping(nounMapping.getWords(), nounMapping.getKind(), claimant, nounMapping.getProbability(), nounMapping.getSurfaceForms());
	}
	...
	/**
	 * Extracts a phrase mapping that has the same phrase as the given phrase
	 * mapping. Currently, we only allow one phrase per phrase mapping to search for
	 * the same phrase.
	 * 
	...
	private INounMapping addNounMappingOrExtendExistingNounMapping(ImmutableList<IWord> words, MappingKind kind, IClaimant claimant, double probability, ImmutableList<String> occurrences) {
		...
		ImmutableList<IPhraseMapping> similarPhraseMappings = currentPhraseMappings
				.select(pm -> SimilarityUtils.getPhraseMappingSimilarity(this, pm, phraseMapping, SimilarityUtils.PhraseMappingAggregatorStrategy.MAX_SIMILARITY) > MIN_COSINE_SIMILARITY);
	...
	private INounMapping addTextualEqualNounMapping(ImmutableList<INounMapping> textualEqualNounMappings, IPhraseMapping phraseMappingWithEqualPhrase, INounMapping nounMapping) {

		if (phraseMappingWithEqualPhrase != null) {

			// contains does not work here
			var equalNounMappingsWithEqualPhrase = textualEqualNounMappings.select(nm -> getNounMappingsByPhraseMapping(phraseMappingWithEqualPhrase).contains(nm));
			...
			ImmutableList<INounMapping> nounMappingsWithSimilarPhrases = textualEqualNounMappings.select(nm -> SimilarityUtils.getPhraseMappingSimilarity(this, getPhraseMappingByNounMapping(nm),
					new PhraseMapping(nounMapping.getPhrases()), SimilarityUtils.PhraseMappingAggregatorStrategy.MAX_SIMILARITY) > MIN_COSINE_SIMILARITY);
	...
	private INounMapping addSimilarNounMapping(IPhraseMapping phraseMappingWithEqualPhrase, ImmutableList<INounMapping> similarMappings, ImmutableList<IPhraseMapping> similarPhraseMappings,
			INounMapping nounMapping) {

		if (phraseMappingWithEqualPhrase != null) {

			ImmutableList<INounMapping> similarNounMappingsWithEqualPhrase = similarMappings.select(nm -> getPhraseMappingByNounMapping(nm).equals(phraseMappingWithEqualPhrase));
	...
	private INounMapping addDifferentNounMapping(IPhraseMapping phraseMappingWithEqualPhrase, ImmutableList<IPhraseMapping> similarPhraseMappings, INounMapping nounMapping) {
	...
	private INounMapping addTextualEqualNounMappingWithEqualPhrase(INounMapping nounMapping, ImmutableList<INounMapping> textualEqualNounMappingsWithEqualPhrase) {

		mergeAllNounMappings(textualEqualNounMappingsWithEqualPhrase, nounMapping);
		return nounMapping;
	}

	private INounMapping addTextualEqualNounMappingWithSimilarPhrase(INounMapping nounMapping, ImmutableList<INounMapping> textualEqualNounMappingsWithSimilarPhrase) {
	...
	/**
	 * Adds a new noun mapping that has a similar phrase mapping as an already
	 * existing noun mapping. However, both phrase mappings do not share same
	 * phrases. Therefore, both can be added freshly to the state.
	 * 
