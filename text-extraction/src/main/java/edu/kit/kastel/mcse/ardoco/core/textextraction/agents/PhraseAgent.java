...
public class PhraseAgent extends PipelineAgent {
    ...
    /*
     * private void createNounMappingIfPhrase(Word word, TextState textState) { var phrase =
     * CommonUtilities.getCompoundPhrase(word);
     * 
     * // if phrase is empty then it is no phrase if (phrase.isEmpty()) { return; } // add the full phrase
     * addPhraseNounMapping(phrase, textState);
     * 
     * // filter NounMappings that are types and add the rest of the phrase (if it changed) var filteredPhrase =
     * CommonUtilities.filterWordsOfTypeMappings(phrase, textState); if (filteredPhrase.size() != phrase.size() &&
     * filteredPhrase.size() > 1) { addPhraseNounMapping(filteredPhrase, textState); } }
     * 
     * private void addPhraseNounMapping(ImmutableList<Word> phrase, TextState textState) { var reference =
     * CommonUtilities.createReferenceForPhrase(phrase); var similarReferenceNounMappings =
     * textState.getNounMappingsWithSimilarReference(reference); if (similarReferenceNounMappings.isEmpty()) { var
     * phraseNounMapping = NounMappingImpl.createPhraseNounMapping(phrase, this, phraseConfidence);
     * textState.addNounMapping(phraseNounMapping, this); } else { for (var nounMapping : similarReferenceNounMappings)
     * { nounMapping.addWords(phrase); nounMapping.setAsPhrase(true); } } }
     */
