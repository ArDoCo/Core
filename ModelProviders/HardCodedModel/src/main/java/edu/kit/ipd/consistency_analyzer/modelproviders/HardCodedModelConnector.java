...
public class HardCodedModelConnector implements IModelConnector {
    ...
    private List<IRelation> matchModeledRelationsWithInstances(List<IInstance> instances, List<ModeledRelation> modeledRelations)
        ...
        for (ModeledRelation mRel : modeledRelations) {
            ...
            IRelation rel = new Relation(getInstanceByUID(instances, instanceUIDs.get(0)), getInstanceByUID(instances, instanceUIDs.get(1)), mRel.getType(),
                    mRel.getUID());
