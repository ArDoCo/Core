...
public class BoxGS extends Box implements Serializable {
   ...
   * @param textBoxes   all {@link TextBox} instances that are directly contained in this box
   *                    (does not include sub boxes!)
   ...
   */
  @JsonCreator
  public BoxGS(@JacksonInject DiagramGS diagram,
               @JsonProperty("boundingBox") BoundingBox boundingBox,
               @JsonProperty("textBoxes") TextBox[] textBoxes,
               @JsonProperty("subBoxes") BoxGS[] subBoxes,
               @JsonProperty("tracelinks") TraceLinkGS[] tracelinks) {
    super(diagram, boundingBox.toCoordinates(), 1,
            Classification.UNKNOWN.getClassificationString(), Arrays.asList(textBoxes),
            Color.BLACK);
  ...
  public String toString() {
    var allText =
            getTexts().stream().map(TextBox::getText).reduce((l, r) -> l + " | " + r).orElse("");
