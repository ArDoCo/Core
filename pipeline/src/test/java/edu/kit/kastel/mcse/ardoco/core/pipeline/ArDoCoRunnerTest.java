...
class ArDoCoRunnerTest {
    ...
    private static void testRunnerDefiningArdoco(ArDoCoRunner runner) {
        ...
        try {
            arDoCo.definePipeline(runner.inputText(), runner.inputModelArchitecture(), runner.inputArchitectureModelType(), runner.inputModelCode(), runner
                    .inputDiagramDir(), ConfigurationHelper.loadAdditionalConfigs(runner.additionalConfigs()));
        } catch (IOException e) {
