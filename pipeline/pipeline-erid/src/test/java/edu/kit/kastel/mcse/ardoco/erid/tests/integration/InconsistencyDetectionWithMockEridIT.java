...
/**
 * Performs the inconsistency detection using ERID with the diagram recognition mock.
 */
public class InconsistencyDetectionWithMockEridIT extends InconsistencyDetectionEvaluationIT {
    @Override
    protected HoldBackRunResultsProducer getHoldBackRunResultsProducer() {
        return new HoldBackRunResultsProducerERID(true);
    }
    ...
    protected void missingModelElementInconsistencyIT(GoldStandardProject goldStandardProject) {
        if (goldStandardProject instanceof GoldStandardDiagramsWithTLR goldStandardDiagramProject)
            runMissingModelElementInconsistencyEval(goldStandardProject, goldStandardDiagramProject.getExpectedMMEResultsWithMock());
        else
