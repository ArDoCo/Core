...
public class PartialSelection<R, M> extends Refactoring<R, M> {
    ...
    public boolean applyTo(AnnotatedGraph<R, M> graph) {
        ...
        Vertex<R> current = this.selectEntry(graph.graph()
                .vertexSet(), vertex -> this.isLeaf(graph, vertex));
        ...
        graph.graph()
                .removeAllVertices(graph.graph()
                        .vertexSet()
                        .stream()
                        .filter(vertex -> !selectedPart.contains(vertex))
                        .collect(Collectors.toList()));

        graph.links()
                .removeIf((vertex, element) -> !selectedPart.contains(vertex));
        graph.inconsistencies()
                .removeIf(inconsistency -> !selectedPart.contains(inconsistency.getBox()));
    ...
    private boolean isLeaf(AnnotatedGraph<R, M> graph, Vertex<R> vertex) {
        return graph.graph()
                ...
                .stream()
                .anyMatch(edge -> edge.getLabel()
                        .equals(Edge.Label.HIERARCHY)) && graph.graph()
