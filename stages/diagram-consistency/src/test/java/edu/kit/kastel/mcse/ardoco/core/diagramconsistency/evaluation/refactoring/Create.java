...
public class Create<R, M> extends Refactoring<R, M> {
    ...
    public boolean applyTo(AnnotatedGraph<R, M> graph) {
        ...
        if (this.getRandom()
                .nextBoolean()) {
            parentVertex = this.selectEntry(graph.graph()
                    .vertexSet());
            if (parentVertex != null && !parentVertex.equals(createdVertex)) {
                graph.graph()
                        .addEdge(createdVertex, parentVertex, new Edge(Edge.Label.HIERARCHY));
            }
        }

        int newEdgeBudget = this.getRandom()
                .nextInt(4);
        if (newEdgeBudget > 0) {
            int incomingEdgeBudget = this.getRandom()
                    .nextInt(newEdgeBudget);
            ...
            for (int i = 0; i < incomingEdgeBudget; i++) {
                Vertex<R> sourceVertex = this.selectEntry(graph.graph()
                        .vertexSet(), vertex -> !blackList.contains(vertex));
                if (sourceVertex != null) {
                    graph.graph()
                            .addEdge(sourceVertex, createdVertex, new Edge(Edge.Label.DEFAULT));
            ...
            for (int i = 0; i < outgoingEdgeBudget; i++) {
                ...
                if (targetVertex != null) {
                    graph.graph()
                            .addEdge(createdVertex, targetVertex, new Edge(Edge.Label.DEFAULT));
