 ...
 */
public record AnnotatedDiagram<M>(Diagram diagram, MutableBiMap<Box, M> links,
                                  Set<Inconsistency<Box, M>> inconsistencies) {
    /**
    ...
    public static <T> AnnotatedDiagram<T> createFrom(String source, AnnotatedGraph<Box, T> graph) {
        ...
        for (Map.Entry<Vertex<Box>, T> entry : graph.links()
                .entrySet()) {
