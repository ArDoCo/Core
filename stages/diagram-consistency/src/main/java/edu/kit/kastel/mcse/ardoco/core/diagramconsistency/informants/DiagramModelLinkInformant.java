...
public class DiagramModelLinkInformant extends Informant {
    ...
    public void run() {
        ...
        DiagramMatchingModelSelectionState selection = data.getData(DiagramMatchingModelSelectionState.ID,
                ...
                .orElse(null);
        DiagramModelLinkState matching = data.getData(DiagramModelLinkState.ID, DiagramModelLinkState.class)
                .orElse(null);
        ...
        for (var selectedModelType : selection.getSelection()) {
            ...
            if (model instanceof ArchitectureModel architectureModel) {
                this.match(diagram.getDiagram(), architectureModel, selectedModelType, matching,
                        selection.getSimilarityFunction());
            } else if (model instanceof CodeModel codeModel) {
                this.match(diagram.getDiagram(), codeModel, selectedModelType, matching,
                        selection.getSimilarityFunction());
            } else {
                this.logger.error("DiagramModelLinkInformant: Unknown model type: {}", model.getClass()
                        .getSimpleName());
            }
        }
    }

    private void match(Diagram diagram, ArchitectureModel model, ModelType type, DiagramModelLinkState matching,
            WeightedTextSimilarity similarityFunction) {
    ...
    private void match(Diagram diagram, CodeModel model, ModelType type, DiagramModelLinkState matching,
            WeightedTextSimilarity similarityFunction) {
        DirectedMultigraph<Vertex<Box>, Edge> diagramAsGraph = Transformations.toGraph(diagram);
    ...
    private <M> void match(ModelType modelType, DiagramModelLinkState matching,
            DirectedMultigraph<Vertex<Box>, Edge> diagramAsGraph, DirectedMultigraph<Vertex<M>, Edge> modelAsGraph,
            Function<M, String> idExtractor, WeightedTextSimilarity similarityFunction) {
        SimilarityFloodingAlgorithm<Vertex<Box>, Vertex<M>, Edge.Label> algorithm = new SimilarityFloodingAlgorithm<>(
                this.epsilon, this.maxIterations, PropagationCoefficientFormula.getInverseAverageFormula(),
                FixpointFormula.getCFormula());

        SimilarityMapping<Vertex<Box>, Vertex<M>> initialSimilarity = new SimilarityMapping<>(pair -> {
            double similarity = similarityFunction.apply(pair.getFirst()
        ...
        SimilarityMapping<Vertex<Box>, Vertex<M>> mapping = algorithm.match(diagramAsGraph, modelAsGraph,
                initialSimilarity);

        OrderedMatchingFilter<Vertex<Box>, Vertex<M>> filter = new OrderedMatchingFilter<>(this.similarityThreshold,
                this.textSimilarityThreshold);
