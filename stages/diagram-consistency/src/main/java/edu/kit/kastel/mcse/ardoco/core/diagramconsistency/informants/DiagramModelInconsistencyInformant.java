...
public class DiagramModelInconsistencyInformant extends Informant {
    private static final List<Rule> BASIC_CONSISTENCY_RULES = List.of(new SameNameForLinkedElements(),
            new AllBoxesMustBeLinked(), new EntitiesMustBeConnectedExactlyToDependencies(), new BoxesMustBeInParent());
    private static final List<Rule> ARCHITECTURE_CONSISTENCY_RULES = addBasicConsistencyRules(
            List.of(new AllModelEntitiesMustBeRepresented()));
    private static final List<Rule> CODE_CONSISTENCY_RULES = addBasicConsistencyRules(
            List.of(new PackagesMustContainAllSubpackagesIfOneIsEmpty()));
    /**
    ...
    public void run() {
        ...
        DiagramModelLinkState matching = data.getData(DiagramModelLinkState.ID, DiagramModelLinkState.class)
                .orElse(null);
        DiagramModelInconsistencyState inconsistencies = data.getData(DiagramModelInconsistencyState.ID,
        ...
        for (var selectedModelType : selection.getSelection()) {
            ...
            Map<Entity, String> entityToId = entities.entrySet()
                    .stream()
