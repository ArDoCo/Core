...
public class MTDEInconsistencyInformant extends Informant {
    ...
    /**
     * Creates an {@link MTDEInconsistency} for each {@link edu.kit.kastel.mcse.ardoco.core.api.diagramrecognition.DiagramElement DiagramElement}
    ...
    public void process() {
        ...
        for (var model : modelIds) {
            ...
            var uncoveredDiagramElements = allDiagramElements.stream().filter(de -> diagramConnectionState.getLinksBetweenDeAndRi(de).isEmpty()).distinct().toList();
