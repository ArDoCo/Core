...
public class TextExtractionTest extends StageTest<TextExtraction, TextExtractionTest.TextProject,
        TextExtractionTest.TextExtractionResult> {
  public TextExtractionTest() {
    super(new TextExtraction(null), TextProject.values());
  }

  @Override
  protected TextExtractionResult runComparable(TextProject project,
                                               Map<String, String> additionalConfigurations,
                                               boolean cachePreRun) {
    var dataRepository = run(project, additionalConfigurations, cachePreRun);

    var wordAbbreviations =
            DataRepositoryHelper.getTextState(dataRepository).getWordAbbreviations();
    var phraseAbbreviations =
            DataRepositoryHelper.getTextState(dataRepository).getPhraseAbbreviations();
  ...
  protected DataRepository runPreTestRunner(TextProject project) {
    return new AnonymousRunner(project.getProjectName()) {
      ...
      public List<AbstractPipelineStep> initializePipelineSteps(DataRepository dataRepository) throws IOException {
        ...
        if (text.isBlank()) {
          throw new IllegalArgumentException("Cannot deal with empty input text. Maybe there was " +
                  "an error reading the file.");
        }
        DataRepositoryHelper.putInputText(dataRepository, text);
        pipelineSteps.add(TextPreprocessingAgent.get(project.getAdditionalConfigurations(),
                dataRepository));

        pipelineSteps.add(ModelProviderAgent.get(project.getModelFile(),
                ArchitectureModelType.PCM, dataRepository));
  ...
  @Override
  protected DataRepository runTestRunner(TextProject project,
                                         Map<String, String> additionalConfigurations,
    ...
    return new AnonymousRunner(project.getProjectName(), preRunDataRepository) {
      ...
      public List<AbstractPipelineStep> initializePipelineSteps(DataRepository dataRepository) {
        var pipelineSteps = new ArrayList<AbstractPipelineStep>();
        pipelineSteps.add(TextExtraction.get(project.getAdditionalConfigurations(),
                dataRepository));
        return pipelineSteps;
      }
    }.runWithoutSaving();
  ...
  public record TextExtractionResult(ImmutableSet<WordAbbreviation> wordAbbreviations,
                                     ImmutableSet<PhraseAbbreviation> phraseAbbreviations) {
  }

  @DisplayName("Evaluate Text Extraction")
  @ParameterizedTest(name = "{0}")
  @EnumSource(value = TextProject.class, mode = EnumSource.Mode.MATCH_NONE, names = "^" +
          ".*HISTORICAL$")
  @Order(1)
  ...
  public enum TextProject implements GoldStandardProject {
    ...
    TEAMMATES( //
            Project.TEAMMATES, //
            List.of(new Disambiguation("GAE", new String[]{"Google App Engine"}),
    ...
    TEASTORE_HISTORICAL( //
            ...
            List.of(new Disambiguation("REST", new String[]{"representational state transfer"}),
                    new Disambiguation("JSP", new String[]{"Java Server Page"}),
                    new Disambiguation("JSPs", new String[]{"Java Server Pages"}),
                    new Disambiguation("OPEN.xtrace", new String[]{"Open Execution Trace " +
                            "Exchange"})) //
    ), //
    TEAMMATES_HISTORICAL( //
            Project.TEAMMATES_HISTORICAL, //
            List.of(new Disambiguation("GAE", new String[]{"Google App Engine"}),
                    new Disambiguation("JSP", new String[]{"Java Server Pages"}),
                    new Disambiguation("POJOs", new String[]{"Plain Old Java Objects"}),
                    new Disambiguation("CRUD", new String[]{"Create Read Update Delete"})) //
    ), //
