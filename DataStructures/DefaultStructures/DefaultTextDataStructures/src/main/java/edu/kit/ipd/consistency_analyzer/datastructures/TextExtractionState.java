...
public class TextExtractionState implements ITextExtractionState {
	...
	private void addOrUpdate(List<INounMapping> nounMappingsWithRef, IWord n, String ref, List<String> occurrences, double hardAddProbability, MappingKind kind) {
		if (nounMappingsWithRef.isEmpty()) {

			List<INounMapping> nounMappingsWithOcc = nounMappings.stream().filter(//
	...
	private void updateKindOfNounMapping(INounMapping nounMapping, MappingKind kind, double probability) {
		...
		List<ITermMapping> termsWithMapping = terms.stream().filter(//
	...
	private boolean hasAnyTermMatchingKind(MappingKind kind, ITermMapping termWithMapping) {
		return termWithMapping.getMappings().stream().anyMatch(mapping -> !mapping.getKind().equals(kind) && !mapping.getKind().equals(MappingKind.NAME_OR_TYPE));
	}
	 ...
	 * {@link #hardAdd(IWord, String, List, MappingKind)} is called and returned. If
	 ...
	 */
	@Override
	public void addTerm(String reference, INounMapping mapping1, INounMapping mapping2, List<INounMapping> otherMappings, MappingKind kind, double probability) {
