...
public class RecommendationState implements IRecommendationState {
	 ...
	 */
	@Override
	public void addRecommendedRelation(String name, IRecommendedInstance ri1, IRecommendedInstance ri2, List<IRecommendedInstance> otherInstances, double probability, List<IWord> occurrences) {
	 ...
	 */
	@Override
	public IRecommendedInstance addRecommendedInstance(String name, String type, double probability, List<INounMapping> nameMappings, List<INounMapping> typeMappings) {
	...
	private void addRecommendedInstance(IRecommendedInstance ri) {
		...
		List<IRecommendedInstance> risWithExactName = recommendedInstances.stream().filter(r -> r.getName().contentEquals(ri.getName())).collect(Collectors.toList());
		List<IRecommendedInstance> risWithExactNameAndType = risWithExactName.stream().filter(r -> r.getType().contentEquals(ri.getType())).collect(Collectors.toList());
